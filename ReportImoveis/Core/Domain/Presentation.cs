...
namespace ReportImoveis.Core.Domain
{
    public class Presentation
    {
        ...
        public float AvaliacaoBase { get; set; }
        public Presentation()
        {
        ...
        public Presentation(List<CriacaoInfoLine> LinesList, Corretor _Corretor, string _Cliente, string _Titulo) 
        {
            foreach (var item in LinesList)
            {
                Imoveis.Add(new Imovel()
                {
                    ...
                    Valor = float.TryParse(item.ValorTxtBox.Text, out var valor) ? valor : 0,
        ...
        public void CalculateAllAvaliation()
        {
            Otimista.CalcularAvaliacao(AvaliacaoBase);
            Otimo.CalcularAvaliacao(AvaliacaoBase);
            Mercado.CalcularAvaliacao(AvaliacaoBase);
        }
    }
    }